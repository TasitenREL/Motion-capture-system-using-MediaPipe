...
このシステムは姿勢推定を行う「Motion-capture-system-using-MediaPipe/pose_post_yolo.py」とサーバーの「Automatic-3D-model-extraction-system-using-LiDAR-and-object-surface-attributes/http_server_json.py」とUnityで3Dモデルを動かす「Automatic-3D-model-extraction-system-using-LiDAR-and-object-surface-attributes/humanPos.cs」の二つコードで動作しています．
